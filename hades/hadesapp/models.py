...
class ArticleComment(models.Model):
    ...
    content = models.TextField(null=False, blank=True, max_length=1000)
    created_at = models.DateTimeField(auto_now_add=True)
...
class Article(models.Model):
    ...
    content = FroalaField()
    games = models.ManyToManyField("Game", related_name="articles", blank=True)
    user = models.ForeignKey(
        "CustomUser", null=True, blank=True, on_delete=models.CASCADE
    )
    cover_picture = models.ImageField(
        null=True, blank=True, upload_to=article_cover_path
    )
    created_at = models.DateTimeField(auto_now_add=True)
...
class CustomUser(AbstractUser):
    ...
    gender = models.CharField(
        max_length=25, choices=GENDER_CHOICES, default=not_selected
    )
